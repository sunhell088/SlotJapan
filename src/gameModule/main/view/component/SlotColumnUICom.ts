 ...
 * slot的纵列
 */
class SlotColumnUICom {
    //后续出现的slot（在没有结果前，是随机，有结果后就是真实结果）
    private residueSlotList:string[] = null;
    //结果slot库
    private slotResultList:SlotResultData[] = null;
    //是否已经把结果加入转动队列
    private bResultJoin:boolean = false;
    //显示中的slot
    private slotDisplayList:SlotPoolObject[] = [];
    //tick的上一帧当前时间戳
    private timeStamp:number = 0;
    //揭晓结果时最后一个slot(用于停止)
    private stopSlot:SlotPoolObject;
    //第几列
    private columnIndex:number;
    //当前列的速度
    private currentSpeed:number = SlotRhythmConst.SPEED;
    //slot容器
    private spinGroup:eui.Group;

    public constructor(spinGroup:eui.Group, columnIndex:number) {
        this.spinGroup = spinGroup;
        this.columnIndex = columnIndex;
    }

    //初始化显示中的slot
    public initSlot(slotIconType:string):void {
        //因为要头尾各加一个，所以这里加2
        var slotMax:number = CommonConst.SLOT_ROW_MAX + 2;
        this.slotDisplayList = [];
        for (var i = 0; i < slotMax; i++) {
            var slotObj:SlotPoolObject = <SlotPoolObject>ObjectPoolManager.getObject(SlotPoolObject);
        ...
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            var slot:SlotPoolObject = this.slotDisplayList[i];
    ...
    //恢复指定的结果
    public recoverLastSlot(result:SlotResultData[]):void {
    ...
    //停止转动，显示结果
    public stopSpin(resultSlots:SlotResultData[], spinAreaUI:SpinAreaUICom):void {
    ...
    private onEnterFrame():void {
        ...
        for (var key in this.slotDisplayList) {
            var slot:SlotPoolObject = this.slotDisplayList[key];
            var distance:number = Math.round((passTime / 1000) * this.currentSpeed);
            //这句做判断是防止每帧位移的距离过大，产生异常（比如设置的速度过快 和 长时间休眠）
            if (distance > (CommonConst.SLOT_ROW_GAP)) distance = (CommonConst.SLOT_ROW_GAP);
            slot.y += distance;
            ...
            if (overValue > 0) {
                ...
                //转动持续时间结束后，开始加入真正的结果
                if (this.residueSlotList.length > 0) {
                    var slotData:any = this.residueSlotList.shift();
        ...
        if (this.stopSlot) {
            ...
            var offset:number = -this.stopSlot.y;
    ...
    //播放格子的动画
    public columnPlaySpecialSlotEffect(slotType:SpecialSlotAnimation[]):void {
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            //排除显示视口外的
            if (!this.slotDisplayList[i].resultData) continue;
            this.slotDisplayList[i].slotPlaySpecialSlotAnimation(true, slotType, false);
        }
    }

    //播放所有的中奖图标闪烁
    public playAllWinSlotBlink():void {
        var forceTopSlotArr:SlotPoolObject[] = [];
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            this.slotDisplayList[i].dark();
            if (!this.slotDisplayList[i].resultData) continue;
            if (!this.slotDisplayList[i].resultData.bWin) continue;
    ...
    //播放单独的一条线图标闪烁
    public playLineWinSlotBlink(blinkLineIndex:number):void {
        var forceTopSlotArr:SlotPoolObject[] = [];
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            if (!this.slotDisplayList[i].resultData) continue;
            ...
            this.slotDisplayList[i].dark();
            var winLineSlot:any[] = Data.globalProxy.winLineSlot[blinkLineIndex];
    ...
    //指定类型格子播放效果
    public playSpecialSlotBlink(slotType:string, bPlay:boolean):void {
        var forceTopSlotArr:SlotPoolObject[] = [];
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            this.slotDisplayList[i].dark();
            if (!this.slotDisplayList[i].resultData) continue;
            this.slotDisplayList[i].resetSlot();
            if (this.slotDisplayList[i].resultData.slotType != slotType) continue;
    ...
    //获得指定索引的显示对象
    public getSlotDisplay(posIndex:number):SlotPoolObject {
        for (var i = 0; i < this.slotDisplayList.length; i++) {
            if (!this.slotDisplayList[i].resultData) continue;
